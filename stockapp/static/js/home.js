...
const socket = new WebSocket(
    ...
    //   console.log(e.data)
        ...
        console.log(Data.message['change'])
        if ( Data.message['change'].toString().indexOf('-')===0) {
        changed.innerHTML = Data.message['change'] // add change in price from json obj 
        Cmp.classList.add('red');
        changed.classList.add('red');
        } 
        else {
            changed.innerHTML ='+' + Data.message['change']// add plus icon if increase
